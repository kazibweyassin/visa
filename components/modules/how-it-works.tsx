    ...
    <section id="how-it-works" className="relative overflow-hidden bg-white py-20 sm:py-28">
      ...
      <div className="relative mx-auto max-w-6xl px-4 sm:px-6 lg:px-8">
        ...
        <div className="mx-auto max-w-2xl text-center">
          <p className="text-[11px] font-medium uppercase tracking-[0.35em] text-[var(--gold)]">How it works</p>
        ...
        <div className="relative mt-16 hidden lg:block">
          {/* Connector line */}
          <motion.div
            className="absolute left-[calc(12.5%+1.5rem)] right-[calc(12.5%+1.5rem)] top-[2.75rem] h-px origin-left bg-gradient-to-r from-blue-200 via-amber-200 to-purple-200"
          ...
          <div className="grid grid-cols-4 gap-6">
            {steps.map((step, index) => (
              ...
              >
                {/* Icon bubble */}
                <div className={`relative z-10 flex h-14 w-14 items-center justify-center rounded-2xl ${step.accent} border ${step.border} shadow-sm`}>
        ...
        <div className="mt-12 space-y-4 lg:hidden">
          {steps.map((step, index) => (
            <motion.div
              ...
              className={`flex gap-4 rounded-2xl border ${step.border} bg-white p-5 shadow-sm`}
            >
              <div className={`relative flex h-12 w-12 shrink-0 items-center justify-center rounded-xl ${step.accent} border ${step.border}`}>
        ...
        >
          <a
            href="/apply"
            className="group inline-flex h-12 items-center justify-center gap-2 rounded-xl bg-[var(--primary)] text-sm font-semibold text-white transition hover:-translate-y-0.5 hover:bg-[var(--secondary)]"
          >
            Start your application
            <ArrowRight className="h-4 w-4 transition-transform duration-200 group-hover:translate-x-0.5" />
          </a>
          <p className="mt-3 text-xs text-slate-400">Takes 5 minutes. No obligation until payment.</p>
        </motion.div>
      </div>
    </section>
  );
