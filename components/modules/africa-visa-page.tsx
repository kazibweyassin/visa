...
const pricingPlans = [
  { key: "basic", title: "BASIC", price: "$49", description: "Document checklist + form review", classes: "bg-white text-slate-900 border border-slate-200" },
  { key: "pro", title: "PRO", price: "$99", description: "Full assistance + interview prep + resubmission guarantee", classes: "bg-[var(--primary)]/5 text-slate-900 border border-[var(--primary)]/20" },
  { key: "express", title: "EXPRESS", price: "$149", description: "Everything in Pro + priority processing + 48hr turnaround", classes: "bg-white text-slate-900 border border-slate-200" },
];
...
export function AfricaVisaPage() {
  ...
  return (
    <div className="bg-white text-slate-900">
      <nav className={`fixed inset-x-0 top-0 z-50 transition ${isSticky ? "border-b border-[#0D1B2A]/10 bg-white/95 shadow-sm backdrop-blur" : "bg-transparent"}`}>
        <div className="mx-auto flex h-20 max-w-7xl items-center justify-between px-4 sm:px-6 lg:px-8">
          <a href="#home" className="flex items-center gap-3">
            <span className="flex h-10 w-10 items-center justify-center rounded-xl bg-[var(--primary)] text-white"><ShieldCheck className="h-5 w-5" /></span>
          ...
          <div className="hidden md:block"><a href="#apply" className="group inline-flex h-12 items-center justify-center gap-2 rounded-xl bg-[var(--primary)] text-sm font-semibold text-white transition hover:-translate-y-0.5 hover:bg-[var(--secondary)]">Apply Now</a></div>
          ...
          {menuOpen && (
            <motion.div initial={{ opacity: 0, y: -8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }} className="border-t border-slate-200 bg-white px-4 py-4 md:hidden">
              <div className="flex flex-col gap-3 text-sm font-semibold">
                ...
                <a href="#apply" onClick={() => setMenuOpen(false)} className="group mt-1 inline-flex h-12 items-center justify-center gap-2 rounded-xl bg-[var(--primary)] text-sm font-semibold text-white transition hover:-translate-y-0.5 hover:bg-[var(--secondary)]">Apply Now</a>
              </div>
            </motion.div>
          )}
        </AnimatePresence>
        ...
        <div className="mx-auto grid max-w-7xl gap-10 px-4 pb-16 sm:px-6 lg:grid-cols-2 lg:px-8">
          ...
          <div className="rounded-3xl bg-white p-6 text-slate-900 shadow-2xl">
            ...
            <div className="mt-5 space-y-4">
              ...
              <button onClick={() => setShowVisaResult(Boolean(fromCountry && toCountry))} className="group inline-flex h-12 w-full items-center justify-center gap-2 rounded-xl bg-[var(--primary)] text-sm font-semibold text-white transition hover:-translate-y-0.5 hover:bg-[var(--secondary)]">Check My Visa <ArrowRight className="h-4 w-4 transition-transform duration-200 group-hover:translate-x-0.5" /></button>
            </div>

            <AnimatePresence>
              {showVisaResult && (
                <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 8 }} className="mt-4 rounded-2xl border border-slate-200 bg-slate-100 p-4">
                  <p className="text-sm font-semibold text-slate-900">You need a Schengen Visa. Processing time: 15 days. Our service fee starts at $79.</p>
                  <a href="#apply" className="group mt-3 inline-flex h-12 items-center justify-center gap-2 rounded-xl bg-[var(--primary)] text-sm font-semibold text-white transition hover:-translate-y-0.5 hover:bg-[var(--secondary)]">Start Application <ArrowRight className="h-4 w-4 transition-transform duration-200 group-hover:translate-x-0.5" /></a>
                </motion.div>
              )}
            </AnimatePresence>
          </div>
      ...
      <section className="bg-slate-50 py-5 text-slate-900">
        <div className="mx-auto grid max-w-7xl grid-cols-2 gap-3 px-4 text-center sm:px-6 md:grid-cols-4 lg:px-8">
          {['10,000+ Visas Processed','98% Approval Rate','54 African Countries Served','24/7 WhatsApp Support'].map((item) => <div key={item} className="rounded-xl bg-white px-3 py-3 text-sm font-semibold text-[var(--primary)]">{item}</div>)}
        </div>
      </section>

      <section id="how-it-works" className="bg-slate-50 py-16 sm:py-20">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <div className="mx-auto max-w-2xl text-center"><h2 className="text-3xl font-extrabold text-slate-900 sm:text-4xl">How It Works</h2></div>
        ...
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          ...
          <div className="mt-8 grid gap-5 sm:grid-cols-2 lg:grid-cols-4">
            {[{ icon: Plane, title: "Tourist Visa", desc: "For leisure trips and family visits." },{ icon: Briefcase, title: "Business Visa", desc: "For meetings, trade, and conferences." },{ icon: GraduationCap, title: "Student Visa", desc: "For admission and academic travel." },{ icon: Route, title: "Transit Visa", desc: "For stopovers and route connections." }].map(({ icon: Icon, title, desc }) => (
              <article key={title} className="rounded-2xl border border-slate-200 p-5"><Icon className="h-7 w-7 text-[var(--primary)]" /><h3 className="mt-3 text-lg font-bold">{title}</h3><p className="mt-2 text-sm text-slate-600">{desc}</p><button className="mt-4 text-sm font-semibold text-[var(--primary)]">Learn More</button></article>
            ))}
          </div>
        </div>
      </section>
          ...
          <div>
            ...
            <div className="mt-6 space-y-4 text-sm sm:text-base">
              {[{ icon: Languages, text: "We speak your language — multilingual support" },{ icon: BadgeCheck, text: "We know African passport challenges — stronger applications" },{ icon: Wallet, text: "Pay with Mobile Money — MTN MoMo, Airtel, M-Pesa, Flutterwave" },{ icon: Smartphone, text: "WhatsApp-first support — talk to a real human" },{ icon: Building2, text: "African-country consulate guides — tailored embassy requirements" }].map(({ icon: Icon, text }) => (
                <div key={text} className="flex items-start gap-3"><Icon className="mt-0.5 h-5 w-5 text-[var(--primary)]" /><p>{text}</p></div>
              ))}
            </div>
          </div>
          ...
          <div className="mt-9 grid gap-5 lg:grid-cols-3">
            {pricingPlans.map((plan) => (
              <article key={plan.key} className={`rounded-3xl p-6 shadow-sm ${plan.classes}`}>
                <div className="flex items-center justify-between"><h3 className="text-lg font-extrabold">{plan.title}</h3>{plan.key === "pro" && <span className="rounded-full bg-[var(--primary)] px-3 py-1 text-xs font-semibold text-white">Best Value</span>}</div>
              ...
              {testimonials.map((item) => (
                <article key={item.name} className="p-2" style={{ flex: `0 0 ${100 / visibleTestimonials}%` }}>
                  <div className="h-full rounded-2xl bg-white p-5 shadow-sm">
                    <p className="text-[var(--primary)]">★★★★★</p>
      ...
      <section id="apply" className="py-16 sm:py-20">
        <div className="mx-auto max-w-4xl px-4 sm:px-6 lg:px-8">
          ...
          <div className="mt-8 rounded-3xl border border-slate-200 p-6 shadow-sm">
            <div className="h-2 overflow-hidden rounded-full bg-slate-200"><div className="h-full bg-[var(--primary)] transition-all" style={{ width: `${(currentStep / 3) * 100}%` }} /></div>
            ...
            <form onSubmit={submitApplication} className="mt-6">
              {currentStep === 1 && (
                <div className="space-y-4">
                  ...
                  <div className="flex justify-end"><button type="button" onClick={nextStep} className="group inline-flex h-12 items-center justify-center gap-2 rounded-xl bg-[var(--primary)] text-sm font-semibold text-white transition hover:-translate-y-0.5 hover:bg-[var(--secondary)]">Next <ArrowRight className="h-4 w-4 transition-transform duration-200 group-hover:translate-x-0.5" /></button></div>
                </div>
              )}
              ...
              {currentStep === 2 && (
                <div className="space-y-4">
                  ...
                  <div className="flex justify-between"><button type="button" onClick={prevStep} className="inline-flex h-11 items-center rounded-xl border border-slate-300 px-5 text-sm font-bold">Back</button><button type="button" onClick={nextStep} className="group inline-flex h-12 items-center justify-center gap-2 rounded-xl bg-[var(--primary)] text-sm font-semibold text-white transition hover:-translate-y-0.5 hover:bg-[var(--secondary)]">Next <ArrowRight className="h-4 w-4 transition-transform duration-200 group-hover:translate-x-0.5" /></button></div>
                </div>
              )}
              ...
              {currentStep === 3 && (
                <div>
                  <div className="grid gap-4 md:grid-cols-3">
                    {pricingPlans.map((plan) => (
                      <button key={plan.key} type="button" onClick={() => setSelectedPlan(plan.title)} className={`rounded-2xl p-4 text-left ${plan.classes} ${selectedPlan === plan.title ? "ring-4 ring-[var(--primary)]/40" : ""}`}>
                  ...
                  <div className="mt-5 flex justify-between"><button type="button" onClick={prevStep} className="inline-flex h-11 items-center rounded-xl border border-slate-300 px-5 text-sm font-bold">Back</button><button type="submit" className="group inline-flex h-12 items-center justify-center gap-2 rounded-xl bg-[var(--primary)] text-sm font-semibold text-white transition hover:-translate-y-0.5 hover:bg-[var(--secondary)]">Proceed to Payment <ArrowRight className="h-4 w-4 transition-transform duration-200 group-hover:translate-x-0.5" /></button></div>
                </div>
              )}
            </form>
            ...
            <AnimatePresence>
              {successReference && (
                <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 8 }} className="mt-4 rounded-xl border border-slate-200 bg-slate-100 p-4 text-sm font-semibold text-slate-900">
      ...
      <Link href="https://wa.me/000000000000" className="fixed bottom-5 right-5 z-40 inline-flex h-12 items-center rounded-full bg-[var(--primary)] px-5 text-sm font-bold text-white shadow-lg">WhatsApp</Link>
    </div>
  );
}
